  ...
  methods: {
    ...
    successUpload(response, uploadFile, uploadFiles) {
      ...
      if (this.res_cnt === uploadFiles.length) {
        ElMessage.warning('文件已上传成功，请耐心等待处理')
        // eslint-disable-next-line no-unused-vars
        this.$axios.get('/overview/process', {params: {title: response}}).then(res => {
          ElMessage.success('处理完成，等待跳转')
          this.$router.push('/overview/' + response)
        })
      }
    },
  },
